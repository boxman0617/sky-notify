...
class SkyNotify
{
    ...
    public static function Subscribe($hook)
    {
...
class PushEMAIL implements iNotifyPush
{
    ...
    public function Deliver($settings)
    {
        $USERS = SkyNotify::$Settings[':user_model'];
...
class PushSMS implements iNotifyPush
{
    ...
    public function Deliver($settings)
    {
        $DATA = SkyNotify::$Settings[':notify_fields']['types_data'];
        $D = $settings->$DATA;
        $MODEL = $D['over'][':model'];
        $FIELD = $D['over'][':field'];
        $obj = new $MODEL();
...
class PushOVER implements iNotifyPush
{
    ...
    public function Deliver($settings)
    {
        ...
        $OPS = array_merge(array(
                "token" => SkyNotify::$Settings[':pushover_token'],
                "user" => $user->$FIELD
        ), $this->params);
